...
func (c *MediaMTXClient) CreatePath(pathName string) error {
	...
	// Note: MediaMTX will auto-create paths when streams are published to them
...
func (c *MediaMTXClient) PathExists(pathName string) (bool, error) {
	...
	if err := json.Unmarshal(body, &pathsList); err != nil {
		// Not critical - MediaMTX will auto-create paths
...
// PublishMetrics publishes metrics to MediaMTX (placeholder for future enhancement)
func (c *MediaMTXClient) PublishMetrics(pathName string, metrics map[string]interface{}) error {
	logger := utils.GetLogger()

	// This is a placeholder for future enhancement
	// MediaMTX has a metrics endpoint, but for Phase 1 we just log
	logger.Debugf("Metrics for path '%s': %+v", pathName, metrics)
